...
**JSON-RPC methods (235):** 24 working, 9 placeholder, 202 forwarded to Hermes (Python). Events emitted: message.start/delta/complete, reasoning.delta, tool.start/complete, session.usage, session.title, status.update, error, gateway.ready. Server requests: approval.
...
**HTTP routes in Hermes (267):** 11 served by the Rust engine; the rest are reverse-proxied to Hermes's Python backend.

| Router | Rust | Forwarded |
|---|---|---|
